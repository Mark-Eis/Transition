...
the main \strong{Transition} package functions \code{add\_prev\_date()},
 \code{add\_prev\_result()} and \code{add\_prev\_transitions()}
 all need to identify the previous observation for that same subject,
 if any. For compatibility with these \strong{Transition} package
 functions, the timings of observations in a dataset, each referred
 to as a \emph{`timepoint`}, should be coded within the data frame
...
\section{Convert numeric values representing year to objects of class \code{"Date"}}
...
We convert numeric the values for year in the \emph{timepoint}
column to class \code{"Date"} using arbitrary values of January
1st for month and day: -
...
We can now add a column of previous test results using the
\code{add\_prev\_result()} function with default values for
...
\section{Convert numeric values representing year and month to objects of class \code{"Date"}}
...
We can now add a column of test result transitions using
\code{add\_transitions()} function with default values
