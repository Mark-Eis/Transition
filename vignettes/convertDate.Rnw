...
be coded within the data frame as \R{} objects of class
...
We start by creating an example data frame of longitudinal data containing years 2018
to 2025 as numeric values for three subjects with observations having one of three
possible ordinal values: -
...
the first argument to add a column of results from the previous observation: -
...
We create another example data frame of longitudinal data containing year and month
July 2024 to June 2025 as numeric values for two subjects with observations having
...
We convert numeric values for year and month to class \code{"Date"}, using
\href{https://stat.ethz.ch/R-manual/R-devel/library/base/html/as.Date.html}
...
We convert the age values to class \code{"Date"}, for the \code{timepoint} argument,
using \href{https://stat.ethz.ch/R-manual/R-devel/library/base/html/as.Date.html} 
{\code{as.Date()}} and an arbitrary ``origin'' of 1st January 2000 to which we add the
age in days\footnote{This works because class \code{"Date"} is represented internally
in days and has a method for the \code{+} operator that returns a date.}, calculated as
years * 365.25.
...
We will now use the \code{add\_prev\_date()} function to add a column of previous test
dates. However, first we need to provide the \code{result} argument, noting that the
\code{exercise} column is formatted neither as the required an ordered factor nor as
binary data with values of either 1 or 0. Since we will not use the \code{exercise}
column for this demonstration, we simply add an integer \code{result} column with
values all 0: -
...
Finally, to be consistent with the original data, we can calculate the previous ages from
the \code{prev\_date} column to include as a \code{prev\_age} column, and remove
the now redundant \code{timepoint}, \code{result} and \code{prev\_date} columns: -

<<>>=
Blackmore <- transform(
        Blackmore,
        prev_age = round(as.integer(prev_date - as.Date("2000-01-01")) / 365.25, 2),
        timepoint = NULL,
        result = NULL,
        prev_date = NULL
    )
...
Note that \R{} formats the \code{age} column to two decimal places, because some
ages in the \code{Blackmore} dataset are not whole numbers of years. However, all
of these non-whole number ages are the last observation for an individual;
consequently, ``previous ages'' are indeed always whole numbers and thus \R{}
formats the \code{prev\_age} column without decimal places.
